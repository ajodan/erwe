                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            ...
                                            <button class="btn btn-danger" data-uri="{{ route('pendidikans.destroy', $res->id) }}" data-toggle="modal" data-target="#deleteConf"><i class="fas fa-trash"></i></button>
                                        </td>
                                    </tr>
                                    @endforeach
                                </tbody>
...
@section('status')
<form method="POST" action="{{ route('pendidikans.store') }}" enctype="multipart/form-data">
@csrf
...
{{ method_field('PUT') }}
    <div class="modal-body">
        <div class="col-12">
            <div class="form-group col-md-12">
                <label>Jenjang Pendidikan</label>
                <input id="jenjang" type="text" class="form-control" name="jenjang" >
            </div>
        </div>
    </div>
        <div class="modal-footer">
...
<script>
    ...
    $(document).on('click', '.edit', function(e){
        ...
        var id = $(this).attr('id');
        $.ajax({
            url: "{{ url('app/getDataPendidikan') }}/"+id,
            dataType: "JSON",
            success:function(data){
                if(data != ""){
                    $('#pendidikan').val(data.jenjang);
                    $('#modalEdit').click();
                    $('#formEditStatus').attr("action", "{{ url('app/pendidikans/') }}/"+id)+"/update";
